"""Code pour Lecture d’un fichier texte structuré"""
# Vos imports ici...
import csv
...
def read_meteo_data(filename):
    ...
    with open(filename, 'r',encoding='utf-8') as file:
        data=[elmt.strip() for elmt in file.readlines()]
    return data
...
def get_meteo_data_by_day(data, station, date):
    ...
    x = date.split(":")
    for i in data:
        if i[0].lower()==station.lower() and x[0]==i[2] and x[1]==i[3] and x[2]==i[4]:
            y=i[5]
            y=float(y)
            temps.append(y)
...
def main():
    """Main pour lire le fichier texte structuré"""
    data = read_meteo_data(DATA)
    print(data)
    data = read_csv_meteo_data(DATA)
    print(data)
